function Param = pre_paramInit2(c,window, noverlap, nfft,pooling,azBound,elBound,gridRes,alphaRes,fs,freqRange,micPos, fre_use)
Param = struct;
%% 
if(isempty(micPos))
...
Param.nfft = nfft;
Param.fs = fs;
% Param.f = (Param.fs/2)/(Param.nfft/2+1)*(1:(Param.nfft/2+1)).';
...
%     Param.freqBins = 1:length(Param.f);
% end
% if (isempty(freqRange)==0)
%     binmul = [];
%     binbin = [];
%     binbinbin = [];
%     if size(freqRange,2)>1
%         for iii = 3 : size(freqRange,2)
%             
%             if mod(iii,2) == 0
%                    if (isempty(find(Param.f<freqRange(iii),1,'last'))==0)
%             binmul(iii) = find(Param.f<=freqRange(iii),1,'last');
%                    end
%             else
%                     if (isempty(find(Param.f >= freqRange(iii),1,'first'))==0)
%             binmul(iii) = find(Param.f >= freqRange(iii),1,'first');
%                     end
%             end
%             
%             if mod(iii,2) == 0
%             binbin = binmul(iii-1):binmul(iii);
%             binbinbin = [binbinbin,binbin];
%             end
%         end
%     end
%     Param.freqBins = binbinbin;
% end
Param.freqBins = fre_use;
...
nMic = size(micPost,1);
Param.pairId = nchoosek(1:nMic,2);     %%%各个阵元的两两组合
