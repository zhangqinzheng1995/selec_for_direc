function [specGlobal,specInst,spec_me] = doa_srp2(x,method, Param)
%% 
...
if strcmp(method,'SRP-PHAT')
    [specGlobal, specInst,spec_me] = ssl_srpPhat2(X,Param);
else
...
function [specGlobal,specInst,spec_me] = ssl_srpPhat2(X,Param)
[~,nFrames,~] = size(X);
specInst = zeros(Param.nGrid, nFrames);
spec_me = zeros(128, nFrames,Param.nGrid);
